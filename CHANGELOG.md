...
- Fixed a startup crash (`PermissionError`) when the log file could not be set up, such as a read-only home directory in a container where `~/.cache` cannot be created, or an existing but unwritable `~/.cache/labelme`; the log file is now best-effort, so a failure to resolve the cache directory (including an unset `LOCALAPPDATA` on Windows), create it, or register the file sink logs a warning and labelme keeps starting with the stderr logging that is already in place ([#2425](https://github.com/wkentaro/labelme/pull/2425))
