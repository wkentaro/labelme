...
- Fixed a startup crash (`TypeError: expected string or bytes-like object`) when the config's `ai.default` is not a string (e.g. `ai:\n  default: true`); the old-model-name migration now only runs on a string value, matching the `shortcuts` guard beside it, so a malformed `ai.default` no longer crashes before the app can start ([#2358](https://github.com/wkentaro/labelme/pull/2358))
