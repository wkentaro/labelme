...
- Fixed a selected label in the Annotation List rendering white-on-pale-gray, and so being unreadable, whenever the list did not hold keyboard focus (most commonly right after selecting a shape on the canvas, which leaves focus on the canvas). The label list's delegate always took its text color from the palette's `Active` group, while the row background beside it is painted by the style from the group implied by the item's state — `Inactive` once the view loses focus — so the two disagreed. The delegate now derives the color group and role exactly as Qt does, matching the File List, which never had the problem ([#2435](https://github.com/wkentaro/labelme/pull/2435))
