...
- Fixed a crash when a `label_flags` config entry contains an invalid regular expression (e.g. `person-(`); such a pattern raised an uncaught `re.error` when loading a labeled shape or typing in the label dialog, and is now skipped instead, with a warning logged naming the offending pattern ([#2350](https://github.com/wkentaro/labelme/pull/2350))
