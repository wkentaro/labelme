...
- Fixed a label flag shared by two matching `label_flags` patterns rendering a duplicate checkbox in the label dialog, and the duplicate silently discarding the user's choice: with a config such as `{cat.*: [occluded, tall], .*: [occluded]}` (the catch-all form the `--label-flags` help text itself suggests), typing `cat` showed two `occluded` boxes, and checking the first one still saved the shape with `occluded: false` because the second, unchecked box overwrote it. Each flag key now renders exactly one checkbox, keeping the position of its first matching pattern ([#2405](https://github.com/wkentaro/labelme/pull/2405))
