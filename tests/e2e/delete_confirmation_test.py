...
_ANNOTATED_FILE_NAME: Final[str] = "annotated/2011_000003.json"
_SHAPE_TIMEOUT_MS: Final[int] = 5_000


def _list_row_labels(win: MainWindow) -> list[str]:
...
def test_shape_deletion_warning_matches_working_undo(
    ...
    win = main_win(
        file_or_dir=str(data_path / _ANNOTATED_FILE_NAME),
    ...
    qtbot.waitUntil(
        lambda: len(canvas.shapes) == shape_count - 1, timeout=_SHAPE_TIMEOUT_MS
    )
    ...
    label_file = tmp_path / Path(_ANNOTATED_FILE_NAME).name
    ...
    qtbot.waitUntil(
        lambda: len(canvas.shapes) == shape_count, timeout=_SHAPE_TIMEOUT_MS
    )

    assert [shape.label for shape in canvas.shapes] == labels
    assert _list_row_labels(win=win) == rows
    with open(label_file) as f:
