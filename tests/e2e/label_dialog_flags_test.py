...
def test_flags_survive_retyping_the_label(
    ...
    # Reopen the shape and retype the same label: the flags shown on open must
    # survive the intermediate keystrokes that match no label_flags pattern.
    item = next(
        it for it in label_list if (s := it.shape()) is not None and s.label == "cat"
    )
    ...
    label_list.item_double_clicked.emit(item)
    qtbot.waitUntil(lambda: not label_dialog.isVisible(), timeout=3000)

    shape = next(s for s in canvas.shapes if s.label == "cat")
    assert shape.flags == {"a": True, "b": False}
