...
def test_empty_state_replaced_after_image_load(
    *,
    ...
    assert win._load_file(
        image_or_label_path=str(data_path / "raw" / "2011_000003.jpg")
    )
...
def test_empty_state_actions_are_keyboard_reachable(
    *,
    ...
    monkeypatch.setattr(
        ...
        dialog_method,
        lambda *_args, **_kwargs: dialog_result(data_path),
    )
...
def _calculate_contrast_ratio(
    *, foreground: QtGui.QColor, background: QtGui.QColor
    ...
    lighter, darker = sorted(
        (
            _calculate_relative_luminance(color=foreground),
            _calculate_relative_luminance(color=background),
        ),
        reverse=True,
    )
